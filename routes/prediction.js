...
router.post("/hp", function (req, res) {
    var URL = process.env.HEART_DIAGNOSIS_TUNNEL ? `${process.env.HEART_DIAGNOSIS_TUNNEL}api` : 'http://127.0.0.1:5050/api';
    ...
    console.log("Sending to Flask:", JSON.stringify({
        "data": [
            arr
        ]
    }));
...
router.post("/dd", function (req, res) {
    var URL = process.env.DIABETESE_DIAGNOSIS_TUNNEL ? `${process.env.DIABETESE_DIAGNOSIS_TUNNEL}api` : 'http://127.0.0.1:5050/api';
    console.log(req.body)
    var arr = [];
    var prediction;
    var name = req.body.name;
